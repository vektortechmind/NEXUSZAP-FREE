          ...
          <Panel className="p-4">
            ...
            <nav className="mt-4 flex gap-2 overflow-x-auto xl:flex-col xl:overflow-visible">
              {INTEGRATION_DOCUMENTATION_TOPICS.map((topic) => (
                <a key={topic.id} href={`#${topic.id}`} className="inline-flex shrink-0 items-center rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 transition-colors hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-800 dark:border-slate-800 dark:bg-slate-950/45 dark:text-slate-200 dark:hover:border-emerald-800 dark:hover:bg-emerald-950/35 dark:hover:text-emerald-300 xl:w-full">
                  {topic.label}
                </a>
              ))}
            </nav>
