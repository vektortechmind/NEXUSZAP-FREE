...
export function Instancia() {
  ...
  const cards = useMemo<ChannelCard[]>(() => {
    const whatsappCards: WhatsappCard[] = instances.map((instance) => ({ key: instance.id, id: instance.id, channel: "WHATSAPP", name: instance.name, status: instance.status, connected: instance.connected, details: instance.occupied && instance.agent ? `Vinculada ao agente ${instance.agent.name}.` : instance.connected ? "Conectada e pronta para uso operacional." : instance.status === "RECONNECTING" ? "Tentando restaurar a conexão do canal." : "Instância criada, sem pareamento ativo no momento.", qr: instance.qr, slot: instance.slot, occupied: instance.occupied, available: instance.available, agent: instance.agent }));
  ...
  const closeDetails = async () => {
    ...
    setSelectedKey(null); setShowTelegramToken(false); setToken("");
  };
  ...
  const handleWhatsappToggle = async (card: WhatsappCard) => {
    ...
    try {
      if (card.connected) { await api.post(`/agent/instances/${card.id}/stop`); clearPairingIntent(card.id); addToast(`Instância ${card.name} desconectada`, "success"); }
      else { markPairingIntent(card.id); openDetails(card); await api.post(`/agent/instances/${card.id}/start`); addToast(`Conectando ${card.name}...`, "info"); }
      await loadData();
    } catch {
  ...
  const handleDeleteInstance = async (card: ChannelCard) => {
